...
If tests use a lot of memory that is not immediately released, using multiprocessing and batching increases the memory footprint of testing. Specifically, using $M$ \acro{CPU}s and batching $N$ test files increases the memory footprint of testing up to $M\cdot N$ times. Therefore, $M$ and $N$ should be capped. For the Markdown package, we use at most 32 \acro{CPU}s and 50 test files.
\looseness=-1
...
\end{verbatim}
Out of the error handling techniques from Section~\ref{sec:error-handling-techniques}, \file{test.sh} could only update test files.
