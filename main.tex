...
When the number of \acro{CPU}s multiplied by the batch size would exceed the number of test files (783), we limit the batch size. The results show what would happen if we did not limit the batch size: As the batch size increases, the number of unused \acro{CPU}s also increases, which decreases testing speed. Whereas with 32~\acro{CPU}s and batch size 32, the median testing time is only 10~minutes, it is 1~hour and 24~minutes with batch size 1024 (about $8.4\times$ slow-down).
...
The techniques of batching and batch splitting were perhaps first used with \TeX{} in the \acro{ARQ}Math shared evaluation tasks for the large-scale indexing of math formulae. In \acro{ARQ}Math-1, the \acro{MIRMU} team used the \LaTeX ML tool to convert \TeX{} formulae to the \acro{XML} format~\cite[Section~2.2]{novotny2020three}. Due to speed issues when processing each formula separately, \acro{MIRMU} processed them in batches. However, large batches led to many formulae being lost due to errors. To mitigate this, \acro{MIRMU} used batch splitting to recover correct formulae.~\cite{novotny2020arqmath} The combination of batching and batch splitting allowed \acro{MIRMU} to convert the formulae quickly and losslessly.
