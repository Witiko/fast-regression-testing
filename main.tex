...
In this article, we describe the testing framework of the Markdown package. In sections~\ref{sec:on-disk-files} and \ref{sec:techniques}, we describe the on-disk files and techniques used in our framework. In Section~\ref{sec:implementation}, we describe our implementation. In sections~\ref{sec:experiments} and \ref{sec:results}, we describe our experiments and their results. In Section~\ref{sec:related-work}, we describe other test frameworks and how they might adapt the techniques used by our framework. We conclude in Section~\ref{sec:conclusion} by summarizing our contributions and outlining future work.
...
We tested the Markdown package at Git commit \texttt{7613632} from August 21, 2023. At this commit, the Markdown package contained 783 test files. For each test file, 10 commands were materialized and executed: four for plain \TeX, four for \LaTeX, and two for \hologo{ConTeXt} MkIV.
...
We ran the experiments for 33 days on a shared Linux server with 400~GB of \acro{RAM} and 80 Intel Xeon Gold 6230 \acro{CPU}s.
...
\begin{align*}
    10\cdot(783\cdot(X + Y)) &= \text{27 hours and 2 minutes} \\
    10\cdot(X + 783\cdot Y) &= \text{1 hour and 16 minutes}
\end{align*}
...
The solution shows that whereas it takes $X\approx\text{12 seconds}$ to initialize a \TeX{} format, it takes only $Y\approx\text{0.5 seconds}$ to process a markdown text. In other words, without batching, 95\% of time is spent on initialization and only 5\% on actual testing; with batching, up to 97\% of time is spent on actual testing.

\subsection{Batch size limiting}
The speed improvements from multiprocessing and batching additive up to a point. Whereas with 1~\acro{CPU} and batch size 1, the median testing time is 27~hours and 2~minutes, it is only 10~minutes with 32~\acro{CPU}s and batch size 32 (about $161\times$ speed-up).

When the number of \acro{CPU}s multiplied by the batch size exceeds the number of test files (783), we cannot use all \acro{CPU}s and the testing speed decreases. Whereas with 32~\acro{CPU}s and batch size 16, the median testing time is only 11~minutes, it is 1~hour and 24~minutes with the same number of \acro{CPU}s and batch size 1024 (about $8\times$ slow-down). Our testing framework prevents this by limiting the batch size.

\section{Related work}
\label{sec:related-work}

In this section, we discuss prior work related to the testing framework of the Markdown package.
...
Unlike our testing framework, the l3build package is written in Lua, which has no support for multiprocessing. Therefore, l3build needs external tools like \acro{GNU}~Parallel to use multiple \acro{CPU}s for testing.

Unlike our testing framework, l3build does not support batching. However, it allows grouping multiple tests in a single file as a form of manual batching.
...
In this study, we explored techniques for speeding up the regression testing of \TeX{} packages. Using multiprocessing and the batching of test files, we improved the testing time of the Markdown package for \TeX{} from hours to minutes.
